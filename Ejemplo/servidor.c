/*Proceso ejemplo para un servidor típico.
 *Para poder correr este proceso abrir la terminal y dirigirse a la carpeta donde se encuentra éste servidor.c
 *y ejectutar el comando: "gcc -o Servidor servidor.c -lcommons"
 *Con este comando se esta llamando al compilador gcc (que ya trae linux), se le dice que cree
 *directamente un ejecutable (con la opcion -o; Ver "man gcc" para mas info), se le dice que
 *el ejecutable se va a llamar Servidor y que use el codigo fuente de servidor.c y además se linkean
 *las commons libraries de la cátedra.
 *Una vez compilado el Servidor, ejecutar en la terminal: "gcc -o Cliente cliente.c -lcommons"
 *para que también cree el ejecutable del cliente.c.
 *Una vez hecho eso, ejecutar en el mismo directorio: "./Servidor".
 *Deberia imprimir el mensaje "Se puso el socket en listen" y quedarse a la espera.
 *Sin cerrar la terminal donde está corriendo el Servidor. Abrir otra terminal y dirigirse a la carpeta
 *donde esta cliente.c y ejecutar: "./Cliente".
 *IMPORTANTE: siempre ejecutar el Servidor primero.
 *Una vez ejecutado el Cliente, los procesos deberian enviar/recibir el mensaje "Acabaste de conectar
 *dos procesos..." para posteriormente cerrarse.
 *Algo copado también para hacer es ejecutar ambos procesos con valgrind (Ver "man valgrind"):
 *"valgrind ./Servidor" y en otra terminal "valgrind ./Cliente".
*/
...
int main(){
	...
	int socket_servidor = crear_socket_servidor("127.0.0.1", "8080");
	...
	*/
        ...
        char *datos_recibidos = recibir_string(socket_aceptado);
        //Recibo los datos del socket y LOS GUARDO EN datos_recibidos
	if(strcmp(datos_recibidos, "fail") == 0){
        //Comparo los datos_recibidos con "fail" en caso de error. Ver "recibir_string()" mas abajo
	    printf("Hubo error al recibir\n");
            free(datos_recibidos);
            close(socket_aceptado);
            close(socket_servidor);
            return 1;
	}
	printf("%s", datos_recibidos); //En caso de que haya salido todo bien muestro el mensaje recibido
        free(datos_recibidos); //Libero la memoria pedida para almacenar el mensaje (Se hace un malloc() en recibir_string)
	close(socket_aceptado); //Cierro la conexion aceptada
        close(socket_servidor); //Cierro el socket de listen. IMPORTANTISIMO CERRAR SOCKETS CUANDO SE TERMINA DE USARLOS
return 0;
}
...
char * recibir_string(int socket_aceptado){ //Esta funcion solo recibe un socket
	...
	if(error_check <= 0){ //Checkeo que recv no haya dado error (<0)
        ...
        while(strcmp(mensaje,"Acabaste de conectar dos procesos. Vamoooo!!!!\n") != 0){
